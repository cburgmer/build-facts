(ns build-facts.jenkins.transform-test
  (:require [build-facts.jenkins.transform :as sut]
...
(deftest test-jenkins-build->build-facts-build
  (testing "should map a passed build"
    (is (= {:job-name "my_job"
           ...
           (sut/jenkins-build->build-facts-build {:job-name "my_job"
                                               :number 42
                                               :result "SUCCESS"
                                               :timestamp 1451877290461
                                               :duration 42}))))
  ...
  (testing "should map a failed build"
    (is (= "fail"
           (:outcome (sut/jenkins-build->build-facts-build (-> a-jenkins-build
                                                            (assoc :result "FAILURE")))))))

  (testing "should extract Git input"
    (is (= [{:source-id "git://some_git"
             :revision "abcd1234"}]
           (:inputs (sut/jenkins-build->build-facts-build (-> a-jenkins-build
                                                           (assoc :actions [{}
  ...
  (testing "should extract build trigger input"
    (is (= [{:job-name "build_name"
             :build-id "42"}]
           (:triggered-by (sut/jenkins-build->build-facts-build (-> a-jenkins-build
  ...
  (testing "should omit build trigger if triggered by user due to temporal disconnect"
    (is (not (contains? (sut/jenkins-build->build-facts-build (-> a-jenkins-build
                                                               (assoc :actions [{:causes [{:upstreamProject "build_name"
                                                                                           :upstreamBuild 42}]}
                                                                                {:causes [{:userId "aUser"}]}])))
                        :triggered-by))))
  ...
  (testing "should omit build trigger if triggered by anonymous user due to temporal disconnect"
    (is (not (contains? (:build (sut/jenkins-build->build-facts-build (-> a-jenkins-build
                                                                       (assoc :actions [{:causes [{:upstreamProject "build_name"
                                                                                                   :upstreamBuild 42}]}
                                                                                        {:causes [{:userId nil}]}]))))
                        :triggered-by))))
  ...
  (testing "should convert test results"
    (is (= [{:name "a test suite"
             :children [{:classname "the class"
                         ...
                         :runtime 42
                         :status "pass"}]}]
           (:test-results (sut/jenkins-build->build-facts-build (-> a-jenkins-build
  ...
  (testing "should convert test statuses"
    (is (= "pass"
           (-> (sut/jenkins-build->build-facts-build (-> a-jenkins-build
