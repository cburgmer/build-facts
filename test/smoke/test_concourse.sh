...
sync_builds() {
    JVM_OPTS="-Duser.home=${SCRIPT_DIR}" "${SCRIPT_DIR}/../../lein" run -m buildviz.concourse.sync mock-auth --from "2021-01-01"
}

ensure_user_agent() {
    local count_request='{"method": "GET", "url": "/api/v1/user", "headers": {"User-Agent": {"matches": "buildviz.*"}}}'
